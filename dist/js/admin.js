...
/* harmony import */ var _components_wysiwyg__WEBPACK_IMPORTED_MODULE_5__ = __webpack_require__(/*! ./components/wysiwyg */ "./assets/js/admin/components/wysiwyg.js");
...
document.addEventListener('DOMContentLoaded', function () {
  ...
  Object(_components_wysiwyg__WEBPACK_IMPORTED_MODULE_5__["default"])();
...
var conditionalValidationInputs = Array.from(document.querySelectorAll('input[data-required-if]'));
var conditionalValidationOther = Array.from(document.querySelectorAll('[data-required-if]:not(input)'));
/**
...
var refreshRequiredAttributes = function refreshRequiredAttributes() {
  conditionalValidationInputs.forEach(function (input) {
    return maybeSetRequired(input);
  });
  conditionalValidationOther.forEach(function (el) {
...
var maybeSetRequired = function maybeSetRequired(input) {
  ...
  input.required = !requiredIf || !current ? false : -1 !== requiredIf.split(',').map(function (s) {
    return s.trim();
  }).indexOf(current);
};
/**
...
var maybeSetRequiredClass = function maybeSetRequiredClass(el) {
  ...
  if (current) {
    if (requiredIf.split(',').map(function (s) {
      return s.trim;
    }).indexOf(current)) {
...
/* harmony import */ var formbouncerjs__WEBPACK_IMPORTED_MODULE_8__ = __webpack_require__(/*! formbouncerjs */ "./node_modules/formbouncerjs/dist/bouncer.polyfills.min.js");
/* harmony import */ var formbouncerjs__WEBPACK_IMPORTED_MODULE_8___default = /*#__PURE__*/__webpack_require__.n(formbouncerjs__WEBPACK_IMPORTED_MODULE_8__);
...
var setupForms = function setupForms() {
  bouncer = new formbouncerjs__WEBPACK_IMPORTED_MODULE_8___default.a('.form--validate', {
    customValidations: {
      ...
      isCloseDateValid: function isCloseDateValid(field) {
        // Bail early.
        if (!field.matches('[name="closeDate[month]"], [name="closeDate[day]"], [name="closeDate[year]"]')) {
          return false;
        }

        var openDay = parseInt(document.querySelector('[name="openDate[day]"]').value);
        var openMonth = parseInt(document.querySelector('[name="openDate[month]"]').value - 1);
        var openYear = parseInt(document.querySelector('[name="openDate[year]"]').value);
        var openDate = new Date(openYear, openMonth, openDay).getTime(); // Bail if the open date isn't valid.

        if (isNaN(openDate)) {
          return false;
        }

        var closeDay = parseInt(document.querySelector('[name="closeDate[day]"]').value);
        var closeMonth = parseInt(document.querySelector('[name="closeDate[month]"]').value - 1);
        var closeYear = parseInt(document.querySelector('[name="closeDate[year]"]').value);
        var closeDate = new Date(closeYear, closeMonth, closeDay).getTime(); // Bail if the close date isn't valid.
      ...
      isDeadlineDateValid: function isDeadlineDateValid(field) {
        ...
        var deadlineDay = parseInt(document.querySelector('[name="deadline[day]"]').value);
        var deadlineMonth = parseInt(document.querySelector('[name="deadline[month]"]').value - 1);
        var deadlineYear = parseInt(document.querySelector('[name="deadline[year]"]').value);
        var deadlineDate = new Date(deadlineYear, deadlineMonth, deadlineDay).getTime(); // Bail if the close date isn't valid.
      ...
      isAwardAmountValid: function isAwardAmountValid(field) {
        ...
        var estimatedAmountFields = document.querySelectorAll('[name="estimatedAmounts[same][amount]"], [name="estimatedAmounts[range][low]"], [name="estimatedAmounts[range][high]"]');
...
var handleBouncerRemoveCloseDateError = function handleBouncerRemoveCloseDateError(event) {
...
var handleBouncerShowFielsetError = function handleBouncerShowFielsetError(event) {
  var field = event.target;
  var fieldset = field.closest('.fieldset--is-required'); // Not a required fieldset? Skip.
...
var handleFormClick = function handleFormClick(event) {
  ...
  var input = form.querySelector('input[name="save"]'); // Update appropriate data.

  input.value = target.getAttribute('data-value'); // Submit.
...
var setupWysiwygs = function setupWysiwygs() {
  var requiredWysiwygs = Array.from(document.querySelectorAll('.wysiwyg[data-required-if]')); // Ensure all the textareas are required.
...
var handleWindowMessage = function handleWindowMessage(event) {
  ...
  var _JSON$parse = JSON.parse(data),
      ...
      dataId = _JSON$parse.dataId,
      textContent = _JSON$parse.textContent;
