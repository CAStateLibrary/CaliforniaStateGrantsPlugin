...
const main = () => {
	form?.addEventListener('submit', function (event) {
		...
		if (
			// Check if it's submit button click other then publish or update button.
			document?.activeElement?.type === 'submit' && // Form submitted with publish/update button.
		...
		if (
			// Check for form on enter field submit event with status other then publish.
			document?.activeElement?.type !== 'submit' && // Form submitted with enter click on any of the form field.
		...
		if (postType?.value && postType.value === CAGrantPlugin.grantAwardSlug) {
			...
			message =
				isForcasted && isForcasted.value === 'forecasted'
					? CAGrantPlugin.l10n.forcastGrantConsent
					: CAGrantPlugin.l10n.activeGrantConsent;
		} else {
