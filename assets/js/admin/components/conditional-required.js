...
const conditionalValidationInputs =
	'input[data-required-if],textarea[data-required-if],input[required],textarea[required]';
...
const requiredPostFinderDiv = Array.from(
	document.querySelectorAll('tr.post_finder_field div[data-post-finder="required"]'),
);
const fundingSources = Array.from(document.querySelectorAll('input[name="fundingSource"]'));
const disbursementMethod = Array.from(
	document.querySelectorAll('input[name="disbursementMethod"]'),
);
...
const main = () => {
	...
	if (visibleElems.length) {
		...
		visibleElems.forEach((input) => {
			...
			if (visibleOptions.fieldId && visibleFields.indexOf(visibleOptions.fieldId) === -1) {
	...
	if (grantTypeInputs.length) {
		...
		grantTypeInputs.forEach((input) =>
			input.addEventListener('change', refreshRequiredAttributes),
		);
	}

	if (startDateElem.length) {
		startDateElem.forEach((input) =>
			input.addEventListener('change', refreshMinMaxDateAttributes),
		);
	}
	...
	if (fundingSources.length) {
		fundingSources.forEach((input) =>
			input.addEventListener('change', refreshFundingNotesRequireClass),
		);
	}

	if (disbursementMethod.length) {
		disbursementMethod.forEach((input) =>
			input.addEventListener('change', refreshFundingMethodNotesRequireClass),
		);
	}
...
const refreshFundingMethodNotesRequireClass = () => {
	...
	if (!disbursementMethod || disbursementMethod.value !== 'other') {
...
const maybeSetHiddenClass = (el) => {
	...
	const isFieldRequiredCheck =
		!requiredIf || !grantType
			? true
			: requiredIf
					.split(',')
					.map((s) => s.trim())
	...
	if (
		...
		|| 'not_empty' === visibleOptions['compare'] && ! current
		|| 'empty' === visibleOptions['compare'] && current
	) {
		el.classList.add('hidden');

		if (isFieldRequiredCheck && visibleOptions.required === true) {
