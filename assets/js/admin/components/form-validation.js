...
const setupForms = () => {
	bouncer = new Bouncer('.form--validate', {
		customValidations: {
			...
			isRangeValid: (field) => {
				...
				if (
					!field.matches(
						'[name*="[low]"], [name*="[high]"], [name*="[first]"], [name*="[second]"]',
					)
				) {
					return false;
				}
				...
				if (
					field === lowField &&
					highField.classList.contains('error') &&
					!lowField.classList.contains('error')
				) {
			...
			isMethodValid: (field) => {
				...
				const submissionMethod = document.querySelector(
					'[name="submission_method"]:checked',
				);
				const pattern = {
				...
				if (submissionMethod && value) {
					...
					if (
						...
						submissionMethod.classList.contains('error')
					) {
			...
			isMatchingFundValid: (field) => {
				...
				if (
					!field.matches(
						'[name="matchingFunds[required]"], [name="matchingFunds[percent]"]',
					)
				) {
					return false;
				}
				...
				const required = document.querySelector(
					'[name="matchingFunds[required]"][value="yes"]',
				);
				...
				if (
					!required.checked &&
					field !== matchedFunding &&
					matchedFunding.classList.contains('error')
				) {
			...
			isAwardAmountValid: (field) => {
				...
				if (
					!field.matches(
						'[name="estimatedAmounts[same][amount]"], [name="estimatedAmounts[range][low]"], [name="estimatedAmounts[range][high]"], [name="estimatedAvailableFunds"], [name="estimatedAmounts[unknown][first]"], [name="estimatedAmounts[unknown][second]"]',
					)
				) {
					return false;
				}

				const totalFundingField = document.querySelector(
					'[name="estimatedAvailableFunds"]',
				);
				const estimatedAmountFields = document.querySelectorAll(
					'[name="estimatedAmounts[same][amount]"], [name="estimatedAmounts[range][low]"], [name="estimatedAmounts[range][high]"],[name="estimatedAmounts[unknown][first]"], [name="estimatedAmounts[unknown][second]"]',
				);
				...
				if (
					field !== totalFundingField &&
					parseInt(estimatedAmount, 10) > parseInt(totalFunding, 10)
				) {
					return true;
				}
				if (field === totalFundingField) {
			...
			isFundingSourceNotesRequired: (field) => {
				...
				if (!fundingSource || fundingSource.value !== 'other') {
					return false;
				}
			...
			isFundingMethodNotesRequired: (field) => {
				...
				const form = field.closest('form');
				const fundingSource = form.querySelector(
					'input[name="disbursementMethod"]:checked',
				);
		...
		messages: {
			...
			isDeadlineDateValid:
				'Invalid date. Please check that the deadline is after the open date.',
			isMethodValid:
				'Please check that this matches submission method and that the URL or email is well formed.',
			isRangeValid:
				'Invalid range. Please check that the first number is lower than the second one.',
			isMatchingFundValid: 'Please set a matched funding percentage.',
			isAwardAmountValid:
				'Please check that this number is lower than the Total Estimated Available Funding.',
			...
			isFundingSourceNotesRequired:
				'Please add funding source notes. ( Required for funding source "Other" )',
			isFundingMethodNotesRequired:
				'Please add funding method notes. ( Required for funding method "Other" )',
	...
	forms.forEach((form) => {
		...
		const inputMaxLimitFields = Array.from(
			form.querySelectorAll('input[data-maxlength]:not(input[pattern])'),
		);
...
const handleBouncerRemoveRangeError = (event) => {
	...
	// Bail early.
	if (
		!field.matches('[name*="[low]"], [name*="[high]"], [name*="[first]"], [name*="[second]"]')
	) {
		return;
	}
